...
try:
    ...
    print("\n=== GATE EVALUATION BETWEEN ALL PAIRS IN CASE 2 ===")
    for item_x in items:
        for item_y in items:
            ...
            # Use forensics if available in modification_report
            ...
            print(f"Match: ID {item_x.id} ({item_x.filename}) vs ID {item_y.id} ({item_y.filename})")
            ...
            print(f"  Standard path passed: {standard_passed}")
            print(f"  Descendant path passed: {descendant_passed}")
