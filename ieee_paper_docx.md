...
* **Threshold 5 (Optimum)**: TP=858, TN=93, FP=907, FN=142, Accuracy=47.55%, Precision=48.61%, Recall=85.80%, F1=62.06%
* **Threshold 10**: TP=622, TN=94, FP=906, FN=378, Accuracy=35.80%, Precision=40.71%, Recall=62.20%, F1=49.21%
* **Threshold 15**: TP=214, TN=144, FP=856, FN=786, Accuracy=17.90%, Precision=20.00%, Recall=21.40%, F1=20.76%
* **Threshold 25**: TP=195, TN=1000, FP=0, FN=805, Accuracy=59.75%, Precision=100.00%, Recall=19.50%, F1=32.64%
* **Threshold 35 (Default)**: TP=11, TN=1000, FP=0, FN=989, Accuracy=50.55%, Precision=100.00%, Recall=1.10%, F1=2.18%
* **Threshold 50**: TP=0, TN=1000, FP=0, FN=1000, Accuracy=50.00%, Precision=0.00%, Recall=0.00%, F1=0.00%

### Table 4: Feature Comparison (Random Forest Validation)
* **Heuristic Engine (T > 35)**: Accuracy=50.55%, Precision=100.00%, Recall=1.10%, F1=2.18%, ROC-AUC=0.2831
* **RF Classifier (Fitted)**: Accuracy=99.50%, Precision=99.50%, Recall=99.50%, F1=99.50%, ROC-AUC=1.0000
* **RF Classifier (JPEG-Only)**: Accuracy=97.75%, Precision=100.00%, Recall=93.64%, F1=96.71%, ROC-AUC=0.9993
* **RF Classifier (Visual-Only)**: Accuracy=65.92%, Precision=52.13%, Recall=44.55%, F1=48.04%, ROC-AUC=0.6607

---

### Figure 1: System Architecture Diagram
1. **Next.js Frontend** sends HTTP POST requests (JSON/FormData) to backend controllers.
2. **FastAPI Controllers** (`app/main.py`) orchestrate upload, query, and DB persistence.
3. **Synchronous DNA Engine** (`app/dna_engine.py`) extracts SHA-256, perceptual hashes (aHash, dHash, pHash), EXIF tags, LSB-entropy, and ELA blockiness.
4. **Asynchronous Processing** calculates semantic CLIP embeddings, queries similar DB items, maps visual containment, and updates primary origin.
5. **Database Storage** persists cases, media items, keyframes, and relationships.

### Figure 2: Evidence Processing Pipeline
1. **Input File Ingestion** triggers verification.
2. **Synchronous Branch** extracts hashes and metadata.
3. **Asynchronous Branch** computes CLIP embeddings, indexes DB, and checks ORB bounding box crop bounds.
4. **Scoring Engine** evaluates cumulative integrity/risk.
5. **Linkage Step** assigns relationship types and estimated origin IDs.

### Figure 3: Relationship Graph Workflow
1. **Target File Ingested** retrieves case families.
2. **Similarity Matrix** computes Hamming distance and CLIP cosine similarity.
3. **Admission Checklist** verifies if combined score >= 0.50 and semantic similarity >= 0.75.
4. **Cluster Assignment** routes target to existing UUID or initializes a new cluster.
5. **Origin Estimation** ranks members (Resolution, EXIF, File size, Chronology) and updates parent-child lineage.

### Figure 4: Variant Detection Pipeline
1. **Matching DNA Profiles** verifies SHA-256 checksum match.
2. **Yes Branch** marks target as Most Probable Origin.
3. **No Branch** runs multi-hash similarity checks, mapping visual containment.
4. **Classification Routing** assigns labels: Cropped Variant, Resized Variant, format/compression changes, or screenshot-derived copy.
